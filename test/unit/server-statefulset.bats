...
@test "server/StatefulSet: server gossip and RPC ports are not exposed by default" {
  ...
  local actual=$(echo "$object" |
      yq -r '.spec.template.spec.containers[0].ports[1].hostPort' | tee /dev/stderr)
  [ "${actual}" = "null" ]

  local actual=$(echo "$object" |
      yq -r '.spec.template.spec.containers[0].ports[2].hostPort' | tee /dev/stderr)
  [ "${actual}" = "null" ]

  # Test that hostPort is not set for rpc ports
  local actual=$(echo "$object" |
      yq -r '.spec.template.spec.containers[0].ports[4].hostPort' | tee /dev/stderr)
  [ "${actual}" = "null" ]
...
@test "server/StatefulSet: server.serflan.port is set to 8301 by default" {
  ...
  local actual=$(echo "$object" |
      yq -r '.spec.template.spec.containers[0].ports[1].containerPort' | tee /dev/stderr)
  [ "${actual}" = "8301" ]

  local actual=$(echo "$object" |
      yq -r '.spec.template.spec.containers[0].ports[2].containerPort' | tee /dev/stderr)
  [ "${actual}" = "8301" ]
